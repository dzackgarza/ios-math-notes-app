...
- **Desktop WebKit has no `PointerEvent.altitudeAngle`/`azimuthAngle`** (undefined). The pointer adapter derives them from `tiltX`/`tiltY` with the Pointer Events Level 3 conversion.
