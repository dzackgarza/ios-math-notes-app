...
- **Kobalte 0.13.14 sets and clears `aria-hidden` on the app root one frame after a modal dialog or menu opens or closes** (`ariaHideOutside`, deferred since kobaltedev/kobalte#700). Scripted steps that open the next overlay at once find the app hidden, and role queries fail. Wait for `#root:not([aria-hidden])` between overlays. Checkbox menu items leave the menu open; Escape closes it.
