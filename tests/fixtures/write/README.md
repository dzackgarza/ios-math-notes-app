...
| `rect-scale-rotate` | Rectangle select (mode 18) of two of four strokes, the bottom-right handle dragged (fixed aspect ratio), select all (`cmd 102`), the top-left handle dragged past the right edge (a negative x scale), then the rotate handle dragged. The scale drags hold the pen button (`mm` 2), so Write scales stroke widths as well. |
